...
./build/wasmlight compile <module.wasm> -o <executable> [--target <triple>] [--connector <file.wlc>]...  # native-executable command (ADR-0015); backends still fail with structured errors until the remaining compiler stages land — never a `.waot`/JIT/interpreter fallback
