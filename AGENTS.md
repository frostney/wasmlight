...
| `source/apps/` | Programs: `wasmlight` (CLI — `inspect` / `validate` / `run` [+ `--aot` / `--no-aot`] / `aot` / `compile`), `wasmlight-shell` (interpreter-free runtime-shell template), `wasmbench` (benchmarks), `wasmspec` (.wast conformance harness, `--tier=interp\|jit\|aot`) || `scripts/` | InstantFPC automation (`stamp-version.pas`) |
