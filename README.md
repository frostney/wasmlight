...
Pre-0.1, early. What is shipped today is the bottom of the stack: the
bounds-checked binary reader with full LEB128 handling, the decoded module
model, and the structural decoder, behind a `wasmlight inspect` command.
The validator, the tier seam, and the host surface are staged in
[docs/roadmap.md](docs/roadmap.md) — that file, not this one, is the
