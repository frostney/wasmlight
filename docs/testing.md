...
- `lwpt test` discovers, compiles, and runs `source/units/*.Test.pas` as
  ...
  callback thunks, `Wasm.Target.Test`,
...
| `Wasm.Jit.CodeBuffer.Test` | The W^X executable-code buffer: emission and byte layout, the label/patch map resolving forward branches, `InsertU32` shifting later labels and patches for Arm64 branch veneers, and the make-executable / cache-flush dance — built on every target, executed only where a backend runs. |
| `Wasm.Jit.Arm64.Test` | The aarch64 backend at the byte level: the A64 encodings checked numerically, large-slot LDR/STR via ADD-scratch, cond/CBZ overflow rewritten to invert+B, the AAPCS64 prologue/epilogue and pinned callee-saved registers, the epoch back-edge check, and the per-op templates — the arch-specific assertions gated on `CPUAARCH64`. |
