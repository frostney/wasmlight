...
| `Wasm.Jit.X64.Test` | The x86-64 backend at the byte level: ModRM/REX/immediate encodings cross-checked against the Intel SDM, the pin map (six callee-saved registers), and the per-op templates — gated on `CPUX86_64`. Wide calls and large slots are admitted, not declined. |
| `Wasm.Jit.Test` | The JIT driver and the differential proof: `JitCanCompile`'s scope fence (EH and unsupported target only), compile-to-buffer, a ≥2048-local frame and a 257-parameter call matching the interpreter, and the arch-agnostic behaviour checks (integer/i64 arithmetic, div/rem traps, float NaN discipline, relops, select, calls and O(1) tail calls) asserted equal to the interpreter, gated on `WASM_JIT_BACKEND` so a no-backend leg still builds. |
