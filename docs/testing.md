...
| `Wasm.Jit.Test` | The JIT driver and the differential proof: `JitCanCompile`'s scope fence (EH, unsupported target, and `return_call*` past the shared tail channel), compile-to-buffer, a ≥2048-local frame and a 257-parameter call matching the interpreter, and the arch-agnostic behaviour checks (integer/i64 arithmetic, div/rem traps, float NaN discipline, relops, select, calls and O(1) tail calls) asserted equal to the interpreter, gated on `WASM_JIT_BACKEND` so a no-backend leg still builds. |
