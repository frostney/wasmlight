...
| `Wasm.Jit.Test` | The JIT driver and the differential proof: `JitCanCompile`'s scope fence, compile-to-buffer, and the arch-agnostic behaviour checks (integer/i64 arithmetic, div/rem traps, float NaN discipline, relops, select, calls and O(1) tail calls, compiled `try_table` catch variants, uncaught `throw` as `EWasmException`, and nested compiled frames) asserted equal to the interpreter, gated on `WASM_JIT_BACKEND` so a no-backend leg still builds. |
