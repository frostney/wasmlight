...
- Delivery branch `codex/optimize-x64-wave4` from exact main `36c9394` (push CI
  run 36197061795 green on all six targets). Evidence root:
  `~/.local/share/wasmlight-evidence/x64-wave4-36c9394`. Method as before;
  schedules ran after `waitquiet.py`, and the final comparison was run one
  workload at a time with a quiet wait before each because other projects'
  builds repeatedly loaded the host. Both lanes were interrupted by an API
  session limit once and restarted/resumed; lane J's first attempt left
  nothing, lane G resumed from committed WIP.
- Profiles: simd vector ops were native SSE but every v128 value round-tripped
  through a slot (16-byte store→load per op), the loop-invariant `v128.const`
  was rebuilt each iteration, and v128 functions were not static-cache
  eligible. gc: `struct.new` went X64RtDispatch → JitDoGc → Allocate /
  TakeCell / StructSet (the threadvar lookups seen in dev samples are
  `fpc_stackcheck`, dev-only).
- Accepted:
  - Lane J `f3839ca` + `d7ca357`: x64 inline `struct.new` for every size class
    (free-list pop, trigger check copied from `Allocate`, bitmap bit, the three
    heap counters, header with mark state, zero-fill of uncovered qwords,
    numeric/packed/ref field fill, destination publish last); helper fallback
    for empty lists, collections, large objects, >32 fields, v128 fields and
    `struct.new_default`. `HeapThreshold` added to the offsets table, the
    published layout (328), both fingerprints and `Wasm.Target.Test`; AOT ABI
    revision 17 -> 18. gc 0.37–0.41 (two rounds, both orders). Also fixed an
    interpreter/ARM64-helper crash: `struct.new` of a field-less struct looped
    `0 .. Integer(N)-1` unsigned (range error dev, access violation release).
  - Lane G `e3c9165`: x64 static cache admits inline v128 ops; v128 values
    live in xmm2–xmm15 with deferred write-back; up to 6 hottest v128
    locals/params get fixed hosts; loop-invariant `v128.const` values hoisted
    into fixed hosts seeded at entry (sole-writer, non-visible temps only;
    zero/all-ones via pxor/pcmpeqd); `local.get` alias forwarding into cached
    vector ops within 4 instructions. No new layout offsets. simd 0.18–0.20,
    simd-long 0.070 (loop body has no memory ops but the epoch load).
  - Combined (J, then G): vs lane-J head simd 5.4x / simd-long 14.4x faster
    (clean reverse run), gc flat, guards flat.
- Final vs wave-4 baseline (per-workload schedules, ms; Wasmtime 47.0.3):
  gc 46.46 -> 17.26 (0.372; Wasmtime 27.31 — now 0.63x Wasmtime),
  simd 9.44 -> 2.37 (0.251; Wasmtime 3.44 — 0.69x Wasmtime); loop, fib,
  call, memory, memory-load, memory-store (15-sample recheck 1.001/1.002),
  memory-grow, startup, host-call(s), 3-arg (15-sample recheck
  0.995/1.024) flat.
...
- Correctness: lane J differential tests (field kinds incl. packed/ref/i31/
  ...
  the empty-struct fix; lane G byte-level tests, a loop-carried v128 epoch
  test, a code-shape test, and a new independent fixture
  `tests/fixtures/wast/x64-v128-cache.wast` (60 assertions, generated by
  `x64-v128-cache.py`) run in all tiers — each killed by the mutations in
  `gates/` (two equivalent survivors in lane G, unreachable after
  validation). Gates: frozen install, format, agents, dev + release builds,
  `lwpt test` 63/63, markdownlint, ARM64 type-check, `x64-writeback.wast`
  81/81 and `x64-v128-cache.wast` 61/61 in every tier (compiled 14), pinned
  core 65,188 in interp/jit/aot (compiled 0/8763/8763) on dev and release,
  non-core identical.
- Status vs goal (x64/Wasmtime, this host): gc 0.63, simd 0.69 (in range);
  host-call 0.52, startup 0.63, memory-grow 0.79 (in or below range);
  loop 1.00, fib 1.18, memory 1.78, memory-store 2.2, memory-load 2.24,
  call 2.04, 3-arg ~5 (still out of range). Next: memory loops (remaining
